...
@Component({
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          @for (course of filteredCourses(); track course.id) {
            ...
            >
              ...
              <div class="flex items-center justify-between border-t border-[var(--color-border)] pt-4 mt-auto gap-2 flex-wrap">
            ...
            <div class="col-span-full text-center py-20 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl">
              <i class="pi pi-filter text-4xl text-[var(--color-text-muted)] mb-3"></i>
              <p class="text-[var(--color-text-secondary)] font-medium">No courses found matching your criteria</p>
            </div>
          }
        </div>
      }
  ...
  styles: `
    ...
    .topic-py-badge {
      background-color: rgba(62, 109, 181, 0.08);
      color: #3e6db5;
    }
    .topic-ma-badge {
...
export class CoursesComponent implements OnInit {
  ...
  getTopicBadgeClass(topic: string): string {
    ...
    if (t.includes('py')) return 'topic-py-badge';
    if (t.includes('ma')) return 'topic-ma-badge';
    if (t.includes('wd') || t.includes('web')) return 'topic-wd-badge';
    if (t.includes('ai')) return 'topic-ai-badge';
