...
@Component({
      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div class="stat-card p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] flex flex-col gap-1">
            ...
            <span class="text-2xl font-extrabold text-emerald-500">{{ countByStatus('Running') }}</span>
          </div>
          <div class="stat-card p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] flex flex-col gap-1">
  ...
  styles: `
    ...
    .status-stopped { background: rgba(245,158,11,0.1); color: #f59e0b; }
    .status-completed { background: rgba(100,116,139,0.1); color: #64748b; }
    .status-archived { background: rgba(100,116,139,0.08); color: #94a3b8; }

    .level-1 { background: rgba(62,109,181,0.12); color: #3e6db5; }
    .level-2 { background: rgba(16,185,129,0.12); color: #10b981; }
    .level-default { background: rgba(100,116,139,0.1); color: #64748b; }
  `,
})
