...
@Component({
  ...
  template: `
    <div class="p-6 md:p-10 max-w-7xl mx-auto min-h-screen">
      <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-[var(--color-text-primary)] tracking-tight">Resources</h1>
        <p class="text-sm text-[var(--color-text-muted)] mt-1">Access course materials, videos, and reference links</p>
      </div>
      <div class="feature-empty-state">
        <div class="empty-icon-ring">
          <i class="pi pi-folder-open text-4xl text-[var(--color-info)]"></i>
        </div>
        <h2 class="text-xl font-bold text-[var(--color-text-primary)] mt-5">No resources yet</h2>
        <p class="text-sm text-[var(--color-text-muted)] mt-2 max-w-xs text-center">
          Study materials, links, and documents shared by instructors will appear here.
        </p>
